...
local function message_user_choice2(cfg, speaker, options, text_input, sound)
	local image, left_side = get_image(cfg, speaker)
	local caption = get_caption(cfg, speaker)
	...
	return function()
		...
		local option_chosen, ti_content = gui.show_ollama(msg_cfg, options, text_input)
...
function wesnoth.wml_actions.ollama(cfg)
	local show_if = wml.get_child(cfg, "show_if") or {}
	if not wml.eval_conditional(show_if) then
		log("[message] skipped because [show_if] did not pass", "debug")
		return
	...
	local msg_dlg = message_user_choice2(cfg, speaker, options, text_input, cfg.sound)
